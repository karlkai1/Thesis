...
try:
    print(f"\n10. Comparing with Status Quo:")
    sq_matrix = np.load("distance_matrix.npy")
    sq_nodes = []
    with open("node_ids.txt", 'r') as f:
        sq_nodes = [line.strip() for line in f]

    print(f"   Status Quo: {len(sq_nodes)} nodes")
    print(f"   MLH: {len(node_ids)} nodes")
    print(f"   Difference: {len(node_ids) - len(sq_nodes)} nodes")

    if len(node_ids) > len(sq_nodes):
        print(f"   ✅ MLH scenario includes MORE delivery points!")
        print(f"      (Better accessibility with cargo bikes)")

except:
    print(f"\n   (Status Quo files not found for comparison)")
